    ...
    <div className="space-y-6">
      ...
      {filteredVideos.length > 0 ? (
        <div className="space-y-8">
          {viewMode === "timeline" ? (
            /* 🌟 時間軸佈局 (中央時間軸 + 左右兩側標記時間 + 橫線延伸) */
            <div className="relative py-4">
              {/* 中央縱向時間軸線 */}
              <div className="absolute left-4 md:left-1/2 top-0 bottom-0 w-0.5 -translate-x-1/2 bg-gradient-to-b from-amber-500/60 via-amber-500/25 to-amber-500/5 shadow-[0_0_10px_rgba(245,158,11,0.3)]" />
              ...
              <div className="space-y-8 md:space-y-12">
                {displayedVideos.map((video, idx) => {
                  ...
                  return (
                    ...
                    >
                      {/* 中央軸心發光節點 */}
                      <div className="absolute left-4 md:left-1/2 -translate-x-1/2 top-6 w-7 h-7 rounded-full bg-[#0a0a0f] border-2 border-amber-500 shadow-[0_0_12px_rgba(245,158,11,0.7)] flex items-center justify-center text-amber-400 z-20">
                        <div className="w-2 h-2 rounded-full bg-amber-400 animate-pulse" />
                      </div>

                      {/* 橫線出去 (Connector Lines) */}
                      {/* 行動版：固定從左軸往右連到卡片 */}
                      <div className="md:hidden absolute left-4 top-9 w-6 h-0.5 bg-gradient-to-r from-amber-500/70 to-amber-500/20" />

                      {/* 桌面版：偶數在左側 (向左延伸橫線)，奇數在右側 (向右延伸橫線) */}
                      {isEven ? (
                        <div className="hidden md:block absolute right-1/2 top-9.5 w-10 lg:w-16 h-0.5 bg-gradient-to-l from-amber-500/80 to-amber-500/10 z-10" />
                      ) : (
                        <div className="hidden md:block absolute left-1/2 top-9.5 w-10 lg:w-16 h-0.5 bg-gradient-to-r from-amber-500/80 to-amber-500/10 z-10" />
                      )}

                      {/* 桌面版雙欄佈局 (左欄與右欄交錯) */}
                      <div
                        className={`w-full grid grid-cols-1 md:grid-cols-2 gap-4 md:gap-12 lg:gap-16 pl-10 md:pl-0 ${
                          isEven
                            ? ""
                      ...
                      >
                        ...
                        <div
                          className={`${
                            isEven ? "md:pr-2 lg:pr-4" : "md:pl-2 lg:pl-4"
                          }`}
                        >
                          <div className="group glass-card rounded-2xl overflow-hidden border border-white/10 hover:border-amber-500/50 transition-all duration-300 shadow-xl hover:shadow-2xl hover:shadow-amber-500/10">
                            {/* 卡片頂部：時間標記與分組 */}
                            ...
                            >
                              <img
                                src={
                                  video.thumbnail || "/cover-placeholder.svg"
                                }
                                alt={video.title}
                                loading="lazy"
                              ...
                              <div className="absolute inset-0 flex items-center justify-center">
                                <div className="w-12 h-12 rounded-full bg-amber-500/90 backdrop-blur-md text-black flex items-center justify-center shadow-lg shadow-amber-500/30 opacity-0 group-hover:opacity-100 group-hover:scale-110 transition-all duration-300">
                                  <Play className="w-5 h-5 fill-current ml-0.5" />
                                </div>
                              </div>

                              {video.ytPrivacyStatus &&
                                video.ytPrivacyStatus !== "public" && (
                                  <div className="absolute top-2.5 right-2.5 px-2 py-0.5 rounded-md bg-black/60 backdrop-blur-md border border-white/10 text-[10px] font-medium text-amber-300">
                                    {video.ytPrivacyStatus === "unlisted"
                                      ? "不公開"
                                      : "私人"}
                                  </div>
                                )}
                        ...
                        <div className="hidden md:flex flex-col justify-center">
                          <div
                            className={`flex items-center ${
                              isEven
                                ? "justify-start text-left pl-3"
                                : "justify-end text-right pr-3"
                            }`}
                          >
                            <div className="inline-flex items-center gap-2 px-4 py-2 rounded-2xl bg-amber-500/10 border border-amber-500/30 text-amber-300 text-xs font-mono font-bold shadow-md shadow-amber-500/10">
